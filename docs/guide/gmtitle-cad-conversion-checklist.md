...
현재 main65 기준에서는 `SWTITLEPREPARE`가 사용 중이 아닌 `DR_A*_Outline` 정의의 raw bbox 위험을 복구 후보로 처리합니다. 이미 도면에 삽입되어 사용 중인 정의는 native link와 위치를 보호하기 위해 자동 교체하지 않고 로그에 남깁니다.
