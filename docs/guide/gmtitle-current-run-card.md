...
```text
로컬 코드 기준: main65 paperset-evidence 이상
현재 LSP 기준: 260704-target-overlap-adopt-main65-paperset-evidence
...
| 로그 문구 | 의미 | 다음 행동 |
| --- | --- | --- |
| `SWTITLEVERSION`이 `260704-target-overlap-adopt-main65-paperset-evidence`가 아님 | 열린 CAD 세션이 예전 LSP를 사용 중 | 변환 금지. `APPLOAD`로 `swcad_load.lsp` 다시 로드 |
...
main65에서 유효한 로그에는 아래 문구가 같이 보여야 합니다.
