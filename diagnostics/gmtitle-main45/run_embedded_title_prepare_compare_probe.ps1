...
if (-not $ProbeDwgPath) {
  ...
  $ProbeDwgPath = Join-Path $workDir ("swtitle_embedded_title_prepare_compare_{0}.dwg" -f $safeLabel)
}
if (-not $LogPath) {
  $safeLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
  $LogPath = Join-Path $workDir ("swtitle_embedded_title_prepare_compare_{0}.txt" -f $safeLabel)
}
...
$fixturePath = Join-Path $PSScriptRoot "embedded_title_prepare_compare_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$safeScriptLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
$scriptPath = Join-Path $workDir ("swtitle_embedded_title_prepare_compare_{0}.scr" -f $safeScriptLabel)
...
$env:SWCAD_EMBEDCMP_LSP = $resolvedLspPath
$env:SWCAD_EMBEDCMP_LABEL = $Label
$env:SWCAD_EMBEDCMP_LOG = $LogPath
$env:SWCAD_EMBEDCMP_CLEAN = if ($RunClean) { "1" } else { "0" }
