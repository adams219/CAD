...
(defun swtitle-stylecmp-create-frame-block (sheet / name dims width height offset left bottom right top)
  ...
  (if (not (swcad-title-block-exists-p name))
    (progn
      ...
      (swtitle-stylecmp-entmake-text
        (swtitle-stylecmp-point (+ left 6.0) (+ bottom 14.0))
        (strcat "Native " (strcase (swcad-title-string sheet)) " built-in title area")
        3.0
      )
      (swtitle-stylecmp-block-end)
    )
...
(defun swtitle-stylecmp-run-clean (/ clean-records delete-result after-records)
  ...
  (list clean-records delete-result after-records)
)
...
(defun swtitle-stylecmp-main (/ lsp-path log-path label handle load-result load-ok version-value fixture-result fixture class-result style-present style-record-result style-records clean-result clean-records clean-delete-result clean-after-records status-result status-ok verify-result verify-ok)
  (setq lsp-path
