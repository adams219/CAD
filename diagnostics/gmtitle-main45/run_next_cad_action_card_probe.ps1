...
Invoke-CardCase `
  ...
  -Expected @("예상 수동 GMTITLE 확인량", "저장된 시트 수량: A2 1장, A3 12장, A4 2장", "지금 필요한 확인", "A3 12장을 처리하기 위한 첫 native 기준 객체 1회", "A4 frame-only 2장은 제목블록 생성 대상이 아닙니다", "Result: READY_FOR_FIRST_NATIVE_GMTITLE", "run_open_workcopy_for_manual_convert.ps1", "SWTITLECONVERTNEXT  (권장: YES/OPEN 반복 응답 자동 선택)", "또는 수동 응답을 직접 고르려면: SWTITLECONVERT", "자동화 경계:", "LSP가 자동 처리: 현재 후보 판별", "사람이 확인: GMTITLE 창의 DR_A*_Outline 용지", "아직 자동화하지 않는 이유: GstarCAD GMTITLE 창이 일반/ISO 기본값으로 열릴 수 있고", "첫 native GMTITLE 기준 객체 1장만 사람이 확인", "SWTITLECONVERTNEXT는 같은 흐름에서 YES/OPEN 같은 반복 응답만 자동 선택합니다", "SWTITLECONVERTNEXT 또는 수동 SWTITLECONVERT 흐름이 GMTITLE 호출", "좌표 입력, 값 복사, 기존 원본 정리는 SWTITLECONVERTNEXT 또는 수동 SWTITLECONVERT 흐름이 자동 처리합니다", "DR_A3_Outline", "SWTITLECONVERTNEXT는 아래 반복 응답 중 현재 상태의 안전한 다음 값만 자동 선택합니다.", "SWTITLECONVERTNEXT를 쓰는 경우 사용자가 YES/OPEN/BATCH/MANUAL을 다시 입력하지 않습니다.", "아래 항목은 수동 SWTITLECONVERT를 쓸 때의 응답 의미를 이해하기 위한 설명입니다.", "YES: 첫 native GMTITLE 1장을 만들고 마무리합니다.", "GMTITLE, TIT, 일반 OPEN을 직접 입력하지 마세요", "SWTITLECONVERT 안에서 물어보는 YES/OPEN/BATCH/MANUAL 응답과 CAD 일반 OPEN 명령은 다릅니다", "긴 좌표를 사람이 직접 치지 마세요", "SWTITLECONVERTNEXT 또는 수동 SWTITLECONVERT 흐름이 GMTITLE 창 뒤에 왼쪽 아래 배치점을 자동 전송합니다", "커서가 화면 중앙에 남아 보여도")
