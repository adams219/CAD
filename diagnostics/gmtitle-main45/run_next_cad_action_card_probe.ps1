...
function New-FakeRepoWorkCopy {
  ...
  Set-Content -LiteralPath (Join-Path $gmtitlePath "swcad_title_scale.lsp") -Encoding ASCII -Value '(setq *swcad-title-scale-version* "260706-convert-next-quoted-pause")'
...
function Write-FakeLog {
  param(
    ...
    [string]$LoadedVersion = "260706-convert-next-quoted-pause",
    [string]$ExpectedVersion = "260706-convert-next-quoted-pause",
