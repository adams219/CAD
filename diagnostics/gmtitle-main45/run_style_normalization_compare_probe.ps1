...
if (-not $ProbeDwgPath) {
  ...
  $ProbeDwgPath = Join-Path $workDir ("swtitle_style_normalization_compare_{0}.dwg" -f $safeLabel)
}
if (-not $LogPath) {
  $safeLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
  $LogPath = Join-Path $workDir ("swtitle_style_normalization_compare_{0}.txt" -f $safeLabel)
}
...
$fixturePath = Join-Path $PSScriptRoot "style_normalization_compare_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$safeScriptLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
$scriptPath = Join-Path $workDir ("swtitle_style_normalization_compare_{0}.scr" -f $safeScriptLabel)
...
$env:SWCAD_STYLECMP_CLEAN = if ($RunClean) { "1" } else { "0" }
$env:SWCAD_STYLECMP_SHEETS = $Sheets
