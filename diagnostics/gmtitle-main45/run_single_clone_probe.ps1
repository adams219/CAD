...
if (-not $ProbeDwgPath) {
  $ProbeDwgPath = Join-Path $workDir "swtitle_single_clone_probe.dwg"
}
if (-not $LogPath) {
  $LogPath = Join-Path $workDir "swtitle_single_clone_probe.txt"
}
...
$fixturePath = Join-Path $PSScriptRoot "single_clone_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$scriptPath = Join-Path $workDir "swtitle_single_clone_probe.scr"
...
$env:SWCAD_SINGLE_CLONE_PROBE_LOG = $LogPath
