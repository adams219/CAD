...
if (-not $ProbeDwgPath) {
  ...
  $ProbeDwgPath = Join-Path $workDir ("swtitle_residue_protection_{0}.dwg" -f $safeLabel)
}
if (-not $LogPath) {
  $safeLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
  $LogPath = Join-Path $workDir ("swtitle_residue_protection_{0}.txt" -f $safeLabel)
}
...
$fixturePath = Join-Path $PSScriptRoot "residue_protection_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$safeScriptLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
$scriptPath = Join-Path $workDir ("swtitle_residue_protection_{0}.scr" -f $safeScriptLabel)
...
$env:SWCAD_RESIDUE_LSP = $resolvedLspPath
$env:SWCAD_RESIDUE_LABEL = $Label
$env:SWCAD_RESIDUE_LOG = $LogPath
