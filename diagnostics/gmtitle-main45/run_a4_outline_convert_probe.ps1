...
if (-not $ProbeDwgPath) {
  $ProbeDwgPath = Join-Path $repoRoot "work\swtitle_a4_outline_convert_probe_260705.dwg"
}
if (-not $LogPath) {
  $LogPath = Join-Path $repoRoot "work\swtitle_a4_outline_convert_probe_260705.txt"
}
...
$fixturePath = Join-Path $PSScriptRoot "a4_outline_convert_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$scriptPath = Join-Path $workDir "swtitle_a4_outline_convert_probe_260705.scr"
...
$env:SWCAD_A4_OUTLINE_CONVERT_LOG = $LogPath
