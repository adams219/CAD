param(
  ...
  [string]$DetailSuffix = "_actual_workcopy_main45_diagnostics",
...
if (-not $ProbeDwgPath) {
  $ProbeDwgPath = Join-Path $repoRoot "work\swtitle_actual_workcopy_main45_status_probe_diagnostics.dwg"
}
if (-not $LogPath) {
  $LogPath = Join-Path $repoRoot "work\swtitle_actual_workcopy_status_main45_diagnostics.txt"
}
...
$scriptPath = Join-Path $workDir "swtitle_actual_workcopy_status_main45_diagnostics.scr"
