...
if (-not $ProbeDwgPath) {
  $ProbeDwgPath = Join-Path $workDir "swtitle_convert_script_guard_probe.dwg"
}
if (-not $LogPath) {
  $LogPath = Join-Path $workDir "swtitle_convert_script_guard_probe.txt"
}
...
$fixturePath = Join-Path $PSScriptRoot "convert_script_guard_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$scriptPath = Join-Path $workDir "swtitle_convert_script_guard_probe.scr"
