...
Assert-Contains -Text $goalStatusText -Needle "Title-missing/frame-only evidence (current sampled source size is A4):" -Label "Goal status generic title-missing evidence heading"
...
Assert-Contains -Text $goalStatusText -Needle "title-missing/frame-only 예외 경로와 현재 A4 샘플의 native 바깥 마커 허용 정책은 검증됨" -Label "Goal status Korean title-missing verified guidance"
