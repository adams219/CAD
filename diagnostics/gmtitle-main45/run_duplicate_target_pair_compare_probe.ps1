...
if (-not $ProbeDwgPath) {
  ...
  $ProbeDwgPath = Join-Path $workDir ("swtitle_duplicate_target_pair_compare_{0}.dwg" -f $safeLabel)
}
if (-not $LogPath) {
  $safeLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
  $LogPath = Join-Path $workDir ("swtitle_duplicate_target_pair_compare_{0}.txt" -f $safeLabel)
}
...
$fixturePath = Join-Path $PSScriptRoot "duplicate_target_pair_compare_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$safeScriptLabel = ($Label -replace "[^A-Za-z0-9_-]", "_")
$scriptPath = Join-Path $workDir ("swtitle_duplicate_target_pair_compare_{0}.scr" -f $safeScriptLabel)
