...
if (-not $ProbeDwgPath) {
  $ProbeDwgPath = Join-Path $workDir "swtitle_a3_status_guidance_probe.dwg"
}
if (-not $LogPath) {
  $LogPath = Join-Path $workDir "swtitle_a3_status_guidance_probe.txt"
}
...
$fixturePath = Join-Path $PSScriptRoot "a3_status_guidance_probe.lsp"
if (-not (Test-Path -LiteralPath $fixturePath)) {
  throw "Fixture LSP not found: $fixturePath"
}

$scriptPath = Join-Path $workDir "swtitle_a3_status_guidance_probe.scr"
...
$env:SWCAD_COMPARE_LABEL = "a3 status guidance"
